...
const Header = () => {
  ...
  return (
    <motion.header
      ...
      className="fixed top-0 left-0 z-50 w-full bg-white transition-all duration-300 ease-in-out shadow-lg"
    >
      <div className="container mx-auto flex items-center justify-between h-full px-4">
        ...
        {/* Hamburger Menu Icon */}
        <button onClick={toggleMenu} className="md:hidden text-2xl text-[#1A374D] cursor-pointer">
          <Menu />
        </button>
      </div>

      {/* Mobile Menu */}
      <nav className={`md:hidden absolute top-full left-0 w-full bg-white shadow-lg transition-all duration-500 overflow-hidden ${isMenuOpen ? 'max-h-96' : 'max-h-0'}`}>
